...
const make_ordered_dictionary_1 = __importDefault(
...
const makeMessagesDictionary = () =>
  (0, make_ordered_dictionary_1.default)(exports.waMessageID);
exports.default = (config) => {
  const socket = config.socket;
  const chatKey = config.chatKey || (0, exports.waChatKey)(true);
  const labelAssociationKey =
    config.labelAssociationKey || exports.waLabelAssociationKey;
  const logger =
    config.logger ||
  ...
  const bind = (ev) => {
    ...
    ev.on(
      ...
      ({
        ...
        if (
          syncType === WAProto_1.proto.HistorySync.HistorySyncType.ON_DEMAND
        ) {
          return; // FOR NOW,
          //TODO: HANDLE
        }
    ...
    ev.on("messages.update", (updates) => {
      ...
      for (const { update, key } of updates) {
        const list = assertMessageList(
          (0, WABinary_1.jidNormalizedUser)(key.remoteJid)
        );
        if (update === null || update === void 0 ? void 0 : update.status) {
          const listStatus =
            (_a = list.get(key.id)) === null || _a === void 0
              ? void 0
              : _a.status;
          if (
            listStatus &&
  ...
  return {
      ...
      if (list && mode === "before" && (!cursorKey || cursorValue)) {
        if (cursorValue) {
          const msgIdx = list.array.findIndex(
            (m) =>
              m.key.id ===
              (cursorKey === null || cursorKey === void 0
                ? void 0
                : cursorKey.id)
          );
    ...
    loadMessage: async (jid, id) => {
      var _a;
      return (_a = messages[jid]) === null || _a === void 0
        ? void 0
        : _a.get(id);
    },
