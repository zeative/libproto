...
const makeChatsSocket = (config) => {
  ...
  const getBotListV2 = async () => {
    ...
    const botList = [];
    for (const section of (0, WABinary_1.getBinaryNodeChildren)(
      botNode,
      "section"
    )) {
      if (section.attrs.type === "all") {
        for (const bot of (0, WABinary_1.getBinaryNodeChildren)(
          section,
          "bot"
        )) {
          botList.push({
  ...
  const updateProfilePicture = async (jid, content, dimensions) => {
    ...
    const { img } = await (0, Utils_1.generateProfilePicture)(
      content,
      dimensions
    );
    await query({
  ...
  const profilePictureUrl = async (jid, type = "preview", timeoutMs) => {
    ...
    try {
      ...
      return (_a =
        child === null || child === void 0 ? void 0 : child.attrs) === null ||
        _a === void 0
        ? void 0
        : _a.url;
    } catch (error) {
      if (
        ...
        logger.info(`[INFO] Profile pic not found for ${jid}, using fallback`);
        return {
          eurl: undefined,
          id: jid.split("@")[0],
          status: null,
