...
const curve_1 = require("@zaadevofc/libsignal").curve;
...
class SenderKeyMessage extends ciphertext_message_1.CiphertextMessage {
  constructor(keyId, iteration, ciphertext, signatureKey, serialized) {
    ...
    if (serialized) {
      ...
      const senderKeyMessage =
        WAProto_1.proto.SenderKeyMessage.decode(message).toJSON();
  ...
  getSignature(signatureKey, serialized) {
    return Buffer.from(
      (0, curve_1.calculateSignature)(signatureKey, serialized)
    );
  }
  serialize() {
